...
controller_interface::InterfaceConfiguration StatusController::command_interface_configuration()
  const
{
...
ar3_controllers::StatusController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
controller_interface::return_type ar3_controllers::StatusController::update(
...
controller_interface::CallbackReturn ar3_controllers::StatusController::on_configure(
...
void StatusController::publish_status()
{
  ...
  for (unsigned int i = 0; i < 6; ++i) {
    status_msg_.limit_switches_triggered[i] =
      static_cast<bool>(state_interfaces_[i + 2].get_value());
  }
  status_pub_->publish(status_msg_);
}

controller_interface::CallbackReturn ar3_controllers::StatusController::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  try {
    // register publisher
    status_pub_ = get_node()->create_publisher<ar3_msgs::msg::Status>(
      "~/ar3_status", rclcpp::SystemDefaultsQoS());
...
controller_interface::CallbackReturn ar3_controllers::StatusController::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  try {
    // reset publisher
