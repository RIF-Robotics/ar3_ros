...
bool TimeoutSerial::isOpen() const { return port.is_open(); }
...
void TimeoutSerial::setTimeout(const boost::posix_time::time_duration & t) { timeout = t; }
...
void TimeoutSerial::read(char * data, size_t size)
{
  ...
  for (;;) {
    ...
    switch (result) {
      ...
      case resultInProgress:  // remain in the loop
        continue;
    }
  }
}
...
void TimeoutSerial::readCompleted(
